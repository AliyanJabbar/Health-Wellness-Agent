...
#     condition = True
#     while condition:
#         print("current_agent_name: ", get_agent(user_context).name)
...
# Enable/disable hooks for debugging
ENABLE_HOOKS = True
...
async def main():
    ...
    # Create config WITHOUT hooks (hooks go to Runner instead)
    config = RunConfig(
        model=external_model, 
        tracing_disabled=True, 
        model_provider=external_client,
    ...
    if ENABLE_HOOKS:
        try:
            run_hooks = HealthWellnessRunHooks()
            print("✅ Hooks enabled successfully")
